...
```{r}
myNormSample <- as_tibble(
  rnorm(n = 30, mean = 50, sd = 10)
)

mean(myNormSample$value)
sd(myNormSample$value)

```

There is more variation in the mean and standard deviation from the input paramters in rnorm() than in the previous example with 1000 samples.
...
generate_n_samples <- function(n) {
  myGeneratedData <- tibble(
    normDistr = rnorm(n, mean = 12, sd = 5),
    expDistr = rexp(n, rate = 0.05),
    chiDistr = rchisq(n, df = 20),
    uniDistr = runif(n, min = 0, max = 15)
  )
}

testSampOne <- generate_n_samples(30) %>% 
                  summarise(normMean = mean(normDistr),
                            expMean = mean(expDistr),
                            chiMean = mean(chiDistr),
                            uniMean = mean(uniDistr),
                            normSD = sd(normDistr),
                            expSD = sd(expDistr),
                            chiSD = sd(chiDistr),
                            uniSD = sd(uniDistr))
...
testSampTwo <- generate_n_samples(1000)%>% 
                  summarise(normMean = mean(normDistr),
                            expMean = mean(expDistr),
                            chiMean = mean(chiDistr),
                            uniMean = mean(uniDistr),
...
```{r}
myDistrib1 <- generate_n_samples(1000)
myDistrib2 <- generate_n_samples(30)

results1 <- sample_mean(myDistrib1)
results2 <- sample_mean(myDistrib2)
```
...
```{r}
multiple_samples <- replicate(1000, generate_n_samples(30), simplify = FALSE)
...
```{r}
purrr::map(1:1000, ~sample_mean(multiple_samples))
